...
net = cv2.dnn.readNetFromCaffe(
    "C:/Users/Joshua/Desktop/Projects/ExamProctoring/deploy.prototxt.txt",
    "C:/Users/Joshua/Desktop/Projects/ExamProctoring/res10_300x300_ssd_iter_140000.caffemodel"
)
...
def load_known_faces(known_faces_dir="known_faces"):
    ...
    for image_name in os.listdir(known_faces_dir):
        ...
        img_encodings = face_recognition.face_encodings(img)  # Get encodings for the image

        # Check if any face encodings were found
        if len(img_encodings) > 0:
            img_encoding = img_encodings[0]  # Get the first encoding
            known_face_encodings.append(img_encoding)
            known_face_names.append(os.path.splitext(image_name)[0])  # Use the image name as the label
        else:
